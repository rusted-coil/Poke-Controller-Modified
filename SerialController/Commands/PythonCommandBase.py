...
class ImageProcPythonCommand(PythonCommand):
    ...
    #  画像認識が通るまで待つだけの関数です。
    #  画面遷移が行われたことを確認する際などに利用することを想定しています。高速な移動をする物体の検知はできないと思います。
    def waitForImageRecognized(self, src, s=200, threshold=0.7):
        print('Image recognition:{}'.format(src), end="")
        count = 0
        while not self.isContainTemplate(src, threshold):
            count += 1
            self.wait(0.25)
            print("." if count % 8 == 0 else "", end="")
            if count > s:
                print('何かがおかしいようです。スクリプトを終了します。 :{}'.format(src))  # force finish
                self.finish()
                return False
        print('Found!')
        return True
