...
class GUI:
    def __init__(self):
        ...
        # open up a camera
        # self.camera = Camera(60)
        self.camera = Camera(self.settings.fps.get())
        self.openCamera()
        ...
        self.fps_cb['values'] = [60, 45, 30, 15, 5]
        self.fps_cb.bind('<<ComboboxSelected>>', self.applyFps)
        self.fps_cb.current(self.fps_cb['values'].index(self.settings.fps.get()))

        # Capture size

        self.label4 = ttk.Label(self.camera_f2, text='Show Size:')
        self.show_size_cb = ttk.Combobox(self.camera_f2, textvariable=self.settings.show_size, width=10,
                                         state="readonly")
        # self.capture_height_cb = [1280,640]
        self.resolution = ["640x360", "1280x720"]
        self.show_size_cb['values'] = self.resolution
        self.show_size_cb.bind('<<ComboboxSelected>>', self.applyWindowSize)

        self.show_size_cb.current(
            self.show_size_cb['values'].index(self.settings.show_size.get())
        )
    ...
    def applyWindowSize(self, event):
        width, height = map(int, self.settings.show_size.get().split("x"))
        print('changed Window Size to: {0} × {1}'.format(height, width))
